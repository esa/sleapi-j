...
/**
 * This class provides the functionality that is specific to CLTU forward
 * service instances for user applications. It is responsible for the creation
 * of operation objects supported by the CLTU service for the user-role.
 * Furthermore it performs pre-processing checks of operation invocations,
 * received from the proxy and the application, to be compatible with the CLTU
 * service. For operation invocations and returns the class performs checks for
 * all service-type specific operation objects received either from the proxy or
 * from the service element. The checks are performed in the functions
 * <operation-type>Inv() or <operation-type>(Rtn(). If the checks are passed,
 * the operations are passed to the state-processing by a call to
 * doStateProcessing(). This includes forwarding of the invocations/returns to
 * the proxy or to the service element.
 */
public class EE_APISE_CLTU_UFSI extends EE_APISE_UFSI implements ISLE_SIOpFactory
{
    /**
     * The constructor, to be used for service instance creation, passes the
     * supplied arguments to the base-class.
     */
    public EE_APISE_CLTU_UFSI(String instanceKey, ISLE_ServiceInform clientIf)
    {
    ...
    {
        T value = super.queryInterface(iid);
        if (value != null)
        {
            return value;
        }
    ...
    public <T extends ISLE_Operation> T createOperation(Class<T> iid, SLE_OpType optype) throws SleApiException
    {
        ...
        T a = opf.createOperation(iid, optype, SLE_ApplicationIdentifier.sleAI_fwdCltu, getVersion());
    ...
    /**
     * Performs all checks on the CLTU-GET-PARAMETER operation supplied by the
     * application. When the checks are completed successfully, state-processing
     * is initiated.
     */
    ...
    /**
     * Performs all checks on the CLTU-GET-PARAMETER return supplied by the
     * proxy. When the checks are completed successfully, state-processing is
     * initiated
     */
    ...
    /**
     * Performs all checks on the CLTU-START operation supplied by the
     * application. When the checks are completed successfully, state-processing
     * is initiated
     */
    ...
    /**
     * Performs all checks on the CLTU-START return supplied by the proxy. When
    ...
    /**
     * Performs all checks on the CLTU-STATUS-REPORT invocation supplied by the
     * proxy. When the checks are completed successfully, state-processing is
     * initiated.
     */
