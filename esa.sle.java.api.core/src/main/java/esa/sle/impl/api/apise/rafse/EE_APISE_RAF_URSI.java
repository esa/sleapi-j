...
/**
 * RAF User Return Service Instance This class provides the functionality that
 * is specific to RAF return service instances for user applications. It is
 * responsible for the creation of RAF service-type specific operation objects
 * that are supported for a user-application. This is provided by the
 * implementation of the interface ISLE_SIOpFactory. For received operation
 * invocations and returns the class checks if the PDU's are compatible with the
 * RAF service type. The class performs RAF service specific checks of operation
 * objects received from the application and the proxy. The class implements
 ...
 * which performs the specific checks. After successful checking the operation
 * invocations/returns state-processing is executed by a call to
 * doStateProcessing().
 */
public class EE_APISE_RAF_URSI extends EE_APISE_URSI implements ISLE_SIOpFactory
{
     ...
     */
    public EE_APISE_RAF_URSI(String instanceKey, ISLE_ServiceInform clientIf)
    {
    ...
    public <T extends ISLE_Operation> T createOperation(Class<T> iid, SLE_OpType optype) throws SleApiException
    {
        ...
        // check operation type supported for a RAF service user SI:
        ...
        T a = opf.createOperation(iid, optype, SLE_ApplicationIdentifier.sleAI_rtnAllFrames, getVersion());
        // no specific set-up for RAF-START required
        // no specific set-up for RAF-GET-PARAMETER required
    ...
    /**
     * Performs all checks on the RAF-START operation supplied by the
    ...
    /**
     * Performs all checks on the RAF-START return supplied by the proxy. When
    ...
    /**
     * Performs all checks on the RAF-GET-PARAMETER return supplied by the
    ...
    /**
     * Performs all checks on the RAF-STATUS-REPORT invocation supplied by the
     * proxy. When the checks are completed successfully, state-processing is
     * initiated.
     */
