 ...
 * instances for user applications. It is responsible for the creation of ROCF
 ...
 * checks if the PDU's are compatible with the RAF service type. The class
 * performs ROCF service specific checks of operation objects received from the
 ...
 * specific checks. After successful checking of the operation
 * invocations/returns state-processing is executed by a call to
 * doStateProcessing().
 */
public class EE_APISE_ROCF_URSI extends EE_APISE_URSI implements ISLE_SIOpFactory
{
     ...
     */
    public EE_APISE_ROCF_URSI(String instanceKey, ISLE_ServiceInform clientIf)
    {
    ...
    {
        T res = super.queryInterface(iid);
        if (res != null)
        {
            return res;
        }
    ...
    public <T extends ISLE_Operation> T createOperation(Class<T> iid, SLE_OpType optype) throws SleApiException
    {
        ...
        // check operation type supported for a ROCF service user SI:
        ...
        T a = opf.createOperation(iid, optype, SLE_ApplicationIdentifier.sleAI_rtnChOcf, getVersion());

        // no specific set-up for ROCF-START required
        // no specific set-up for ROCF-GET-PARAMETER required
    ...
    /**
     * Performs all checks on the ROCF-START operation supplied by the
    ...
    /**
     * Performs all checks on the ROCF-START return supplied by the proxy. When
    ...
    /**
     * Performs all checks on the ROCF-GET-PARAMETER return supplied by the
    ...
    /**
     * Performs all checks on the ROCF-STATUS-REPORT invocation supplied by the
     * proxy. When the checks are completed successfully, state-processing is
     * initiated.
     */
